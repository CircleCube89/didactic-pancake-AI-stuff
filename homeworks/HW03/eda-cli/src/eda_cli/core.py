...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    flags["has_constant_columns"]=1 if True in [ i.unique == 1 for i in summary.columns] else False
    flags['has_high_cardinality_categoricals'] = 1 if True in [ i.unique > 30 for i in summary.columns] else False
    if summary.columns[0].is_numeric:
        flags['has_missing_id_rows'] = (((summary.columns[0].min+summary.columns[0].max )//2) != int(summary.columns[0].mean)) 
        
    else:
        flags['has_missing_id_rows'] = False
